**一种特殊的 Volume: Projected Volume，你可以把它翻译为“投射数据卷”。**
...
$ k get secrets
NAME                                 TYPE                                  DATA   AGE
default-token-wprn6                  kubernetes.io/service-account-token   3      14d
nfs-client-provisioner-token-xnhfm   kubernetes.io/service-account-token   3      5d20h
user                                 Opaque                                1      64s
pass                                 Opaque                                1      42s
```
...
k create -f test-projected-volume.yaml
```
验证一下这些 Secret 对象是不是已经在容器里了：
```
$ k exec -it test-projected-volume -- /bin/sh
$ ls /projected-volume/
user
pass
...
$ k create configmap ui-config --from-file=./ui.properties

# 查看这个ConfigMap里保存的信息(data)
$ k get configmaps ui-config -o yaml
apiVersion: v1
data:
  ui.properties: |
    color.good=purple
...
```
$ k create -f dapi-volume.yaml
$ k logs test-downwardapi-volume
cluster="test-cluster1"
rack="rack-22"
zone="us-est-coast"
```
...
在这个 Pod 里安装一个 Kubernetes 的 Client，这样就可以从容器里直接访问并且操作这个 Kubernetes 的 API 了。
## API Server 的授权
...
创建这个 Pod：
```
$ k create -f test-liveness-exec.yaml
```
查看这个 Pod 的状态：
```
$ k get pod
...
```
$ k describe pod test-liveness-exec
```
